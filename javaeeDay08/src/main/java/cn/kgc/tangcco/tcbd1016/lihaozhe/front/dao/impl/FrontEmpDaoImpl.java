...
public class FrontEmpDaoImpl implements FrontEmpDao {
	...
	@Override
	public Emp selectEmpByUuid(String uuid) throws SQLException {
		StringBuilder sql = new StringBuilder(" SELECT e.`id`,e.`account`,e.`password`,e.`nickname`, e.`birthday`,e.`gender`,e.`powernum`,e.`uuid`,e.`did`, d.`name` FROM emp as e INNER JOIN dept as d ON 1 = 1 ");
		sql.append(" AND e.did = d.id  ");
		sql.append("  AND uuid = ?  ");
		List<Emp> empList = new ArrayList<Emp>();
		// 从数据源获取数据库连接
		...
		rs = BaseDBUtils.executeQuery(pst,uuid);
		if (rs != null) {
			if (rs.next()) {
				return new Emp(rs.getInt("id"),rs.getString("nickname"), rs.getString("account"),rs.getString("password"),rs.getDate("birthday"), rs.getInt("gender"), rs.getInt("powernum"), rs.getString("uuid"),rs.getInt("did"), new Dept(rs.getString("name")));
			}
		}
		// 返回有查询参数的的结果
		return null;
	...
	@Override
	public long selectCountByAccount(String account) throws SQLException {
		StringBuilder sql = new StringBuilder("SELECT COUNT(`id`) FROM emp WHERE  1 = 1 ");
		sql.append("  AND account = ?  ");
		List<Emp> empList = new ArrayList<Emp>();
		return queryRunner.query(BaseDBUtils.getConnection(), sql.toString(), account, new ScalarHandler<Long>(1));
	}
