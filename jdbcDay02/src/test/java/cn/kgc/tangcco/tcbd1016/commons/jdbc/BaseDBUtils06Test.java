...
public class BaseDBUtils06Test {
	@Test
	public void test01() {
		...
		int i = 0;
		try {
			conn = BaseDBUtils06.getConnection();
			pst = BaseDBUtils06.getPreparedStatement(conn, sql.toString());
			i = BaseDBUtils06.executeUpdate(pst, parma);
			Emp emp = null;
			if (i > 0) {
			...
			try {
				BaseDBUtils06.closeAll(conn, pst, null);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
